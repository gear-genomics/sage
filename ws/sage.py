...
def allowed_file(filename):
   return '.' in filename and filename.rsplit('.', 1)[1].lower() in set(['ab1', 'abi', 'ab', 'json', 'fa'])
...
def download(uuid):
   if is_valid_uuid(uuid):
      filename = "sage_" + uuid + ".json"
      if allowed_file(filename):
         sf = os.path.join(app.config['UPLOAD_FOLDER'], uuid[0:2])
         if os.path.exists(sf):
            if os.path.isfile(os.path.join(sf, filename)):
               with open(os.path.join(sf, filename), "r") as jsfile:
                  jscon = ""
                  for line in jsfile:
                     jscon += line
                  results = jscon + "; //"
                  return render_template('index.html', baseurl = app.config['BASEURL'], results = results)
   error = "File does not exist!"
   return render_template('index.html', baseurl = app.config['BASEURL'], error = error)
...
if __name__ == '__main__':
   parser = argparse.ArgumentParser(description='Sage App')
